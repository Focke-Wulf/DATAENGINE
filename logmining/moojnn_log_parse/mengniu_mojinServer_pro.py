...
logRunerConf = {
    'mengniu_mojinServer_pro_user':['mengniu_mojinServer_pro',
                                   ],
}
    ...
    with open(log,'r') as f:
        for i in f.readlines():
            ...
            try:
                ...
                row = [year,month,day,timez,intercepter,logLevel,http_methods,api,remoteHost,remoteIP,webaddress,system,browser_CORE,browser,cookie,email,user_id,moojnn_version,license,action]
                column.append(row)
            except:
                print("ERROR :",x)
                continue
...
def pars_runer():
    ...
    for value in logRunerConf:
        header_name = ['year','month','day','timez','intercepter','logLevel','http_methods','api','remoteHost','remoteIP','webaddress','system','browser_CORE','browser','cookie','email','user_id','moojnn_version','license','action']
        header_type = ['text','text','text','text','text','text','text','text','text','text','text','text','text','text','text','text','text','text','text','text']
        try:
